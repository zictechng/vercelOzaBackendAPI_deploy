  ...
  try {
    ...
    // check if user enabled in-app notifications and send notification
      ...
      try {
        ...
        const rejectionEmail = {
          from: { name: `${appName} Support`, email: `<${appSettings.app_email || 'noreply@ota.com'}>` },
          to: [{ email: userDetail.email }],
...
router.get("/reports/financial", isAuth, async (req, res) => {
  try {
    ...
    const [totalCredits, totalDebits, pendingTx, completedTx, failedTx, timeSeries, categoryBreakdown, billsRevenue] = await Promise.all([
      ...
      TransferFund.aggregate([
        { $match: { creditOn: { $gte: startDate, $lte: endDate }, tran_type: 'Debit', transaction_status: { $in: ['Completed', 'Successful', 'successful'] } } },
        { $group: { _id: null, total: { $sum: '$amount' }, count: { $sum: 1 } } }
      ]),
...
router.get("/reports/users", isAuth, async (req, res) => {
  try {
    ...
    const newUsersInPeriod = await User.countDocuments({ createdOn: { $gte: startDate, $lte: endDate } });
    const verifiedUsers = await User.countDocuments({ acct_approved_status: 'Approved' });
...
router.get("/reports/export", isAuth, async (req, res) => {
  try {
    ...
    const transactions = await TransferFund.find(filter)
      .sort({ creditOn: -1 })
      .limit(5000)
      .select('acct_name acct_number tid amount tran_type transac_nature transac_category transaction_status creditOn pay_tran');

    const exportData = transactions.map(tx => ({
      'Name': tx.acct_name || '',
      'Account/Tag': tx.acct_number || '',
      'Transaction ID': tx.tid || '',
      'Amount (NGN)': tx.amount || 0,
      'Type': tx.tran_type || '',
      'Nature': tx.transac_nature || '',
      'Category': tx.transac_category || '',
      'Status': tx.transaction_status || '',
      'Reference': tx.pay_tran || '',
      'Date': tx.creditOn ? new Date(tx.creditOn).toISOString().split('T')[0] : '',
...
router.post("/approveUsdFunding", isAuth, async (req, res) => {
  try {
    ...
    // Credit usd_balance + update tran_account record keeper
    ...
    fetchApp().then(async (result) => {
      try {
        ...
        console.log('USD fund approval email error:', emailErr.message);
      }
    }).catch(console.error);
